...
# @@Created          :  Friday, Jun 26, 2026 11:57 EDT
# @@File             :  zz-nginx.sh
# @@Description      :  
...
if [ -n "$ZZ_NGINX_SH_APPNAME_ENABLED" ]; then
  if [ "$ZZ_NGINX_SH_APPNAME_ENABLED" != "yes" ]; then
...
# command to execute
EXEC_CMD_BIN='/usr/sbin/nginx'
# command arguments - daemon off; is set in nginx.conf so no extra flags needed
EXEC_CMD_ARGS=''
...
# root user name
root_user_name="${ZZ_NGINX_SH_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${ZZ_NGINX_SH_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${ZZ_NGINX_SH_USER_NAME:-}"
# normal user password
user_pass="${ZZ_NGINX_SH_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# Set SUPERSYNC_BASE_URL to point at a self-hosted supersync server;
# leave empty to use the public https://sync.super-productivity.com
SUPERSYNC_BASE_URL="${SUPERSYNC_BASE_URL:-}"
...
__run_precopy() {
  ...
  # Seed /config/nginx from /etc/nginx on first run and symlink back
  if builtin type -t __init_nginx | grep -q 'function'; then
    __init_nginx
  fi
  # Ensure nginx monitors alongside tini in the __no_exit health loop
  export SERVICES_LIST="${SERVICES_LIST:+$SERVICES_LIST }nginx"
...
__update_conf_files() {
  ...
  # Write to vhosts.d — seeded from rootfs/tmp/etc/nginx/vhosts.d/ and included by nginx.conf
  mkdir -p "/etc/nginx/vhosts.d"
  nginx_conf="/etc/nginx/vhosts.d/default.conf"

  # Write the nginx vhost config with runtime values substituted
  {
    echo "server {"
    ...
    echo ""
    echo "    # SPA routing - return index.html for all Angular routes"
    echo "    location / {"
...
__pre_execute() {
  ...
  local sysname="${SERVER_NAME:-${FULL_DOMAIN_NAME:-$HOSTNAME}}"
  local wwwroot="${WWW_ROOT_DIR:-/usr/local/share/httpd/default}"
  local sync_config="${wwwroot}/assets/sync-config-default-override.json"

  # Seed sync provider pre-config from environment variables if jq is available
  # and at least one relevant env var is set. super-productivity reads this file
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    if [ -n "${SUPERSYNC_BASE_URL:-}" ]; then
      json=$(printf '%s' "$json" | jq '.syncProvider = "SuperSync"')
      json=$(printf '%s' "$json" | jq --arg v "$SUPERSYNC_BASE_URL" '.superSync.baseUrl = $v')
    fi

    # Shared sync settings (apply regardless of provider)
    if [ -n "${SYNC_INTERVAL:-}" ]; then
      # App expects milliseconds; env var is in minutes
      local sync_ms=$(( SYNC_INTERVAL * 60000 ))
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$ZZ_NGINX_SH_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$ZZ_NGINX_SH_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$ZZ_NGINX_SH_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$ZZ_NGINX_SH_USER_PASS_WORD}"             #
