...
# @@Created          :  Friday, Jun 26, 2026 11:56 EDT
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for super-productivity
...
#SERVICE_USER="${SERVICE_USER:-super-productivity}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-super-productivity}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    if [ "$ENTRYPOINT_MESSAGE" = "yes" ]; then
      echo "Executing entrypoint script for super-productivity"
    fi
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
